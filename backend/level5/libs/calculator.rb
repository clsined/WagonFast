...
class Calculator
  ...
  def calculate(cars_array, rentals_array)
    ...
    rentals_array.each do |rental|
      ...
      curr_calc_rental = OutputRentals.new(rental[1].rental_id, cars_array[rental[1].car_id].price_per_day, rental[1].duration, cars_array[rental[1].car_id].price_per_km, rental[1].km, rental[1].deductible_reduction)
